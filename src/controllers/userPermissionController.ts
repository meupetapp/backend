...
import { findPetsByUserId } from "../services/userPermissionService";
...
export const listUserPermissions = async (req: FastifyRequest, reply: FastifyReply) => {
  try {
    const user = await findUserByToken(req.headers.authorization || '');
    ...
    const userPermissions = await findPetsByUserId(user.id);
